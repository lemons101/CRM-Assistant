...
### 1. 固定两表模型
- Customers（长期客户画像，按客户身份 upsert）
- OpportunitySnapshots（每次会议一条快照，保留推进轨迹）

### 2. Customers 更新规则
对 Customers 的所有字段统一执行：
...
- 同一客户多轮推进时，Customers 做增量更新，商机快照表持续追加
- 若用户特别在意正式客户ID/商机ID继承，需额外关注是否命中现有飞书记录并优先沿用已有正式 ID
